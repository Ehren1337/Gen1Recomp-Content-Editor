...
- `pokemon.received`: Gold has no received-mon seam of its own yet.
...
Groups that used to sit here have since landed and moved to the shared table
above: the frame seams (`render.compose` / `render.hud` /
...
`pokemon.icon`), the catch/evolution trio (`pokemon.caught`,
...
`src/core/gen2/Evolution.lua` emits `pokemon.evolved` from `apply` and wraps
each row's decision in `evolution.check`), and `pokemon.before_give`
(`src/world/gen2/World.lua` givePoke, same mutable
`{ ctx, species, level, nickname? }` payload as Gen 1 `Commands.give_pokemon`).
