...
local function lastValidateLines(text, n)
  ...
  local tail = {}
  for i = #lines - n + 1, #lines do tail[#tail + 1] = lines[i] end
  return tail
end
...
function Project.draw(S, x, y, w, h, App)
  ...
  Kit.caption(viewX, fy, "VALIDATE / PLAYTEST")
  fy = fy + 24 * s
  local btnW = 120 * s
  if Kit.button(viewX, fy, btnW, fh, "Validate", {
      kind = "primary",
      tooltip = "Run python tools/modkit.py validate on this mod",
    }) then
    if App.validateMod then
      App.validateMod()
    else
      S.status = "Implement App.validateMod() to run modkit validate from the editor"
    end
  end
  if Kit.button(viewX + btnW + 10 * s, fy, btnW, fh, "Playtest", {
      kind = "accent",
      tooltip = "Enable this mod in options and launch the game",
    }) then
    if App.playtestMod then
      App.playtestMod()
    else
      S.status = "Implement App.playtestMod() to launch a playtest build"
    end
  end
  fy = fy + fh + 10 * s

  if S.validateOutput and S.validateOutput ~= "" then
    local lines = lastValidateLines(S.validateOutput, 8)
