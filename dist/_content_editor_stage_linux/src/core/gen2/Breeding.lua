...
-- Optional content-editor overrides on data.breeding (denominator-style knobs
-- for Day-Care / hatch).  Missing keys keep the cart constants above.
function Breeding.knobs(data)
  local b = data and data.breeding
  if type(b) ~= "table" then b = nil end
  local function num(key, default)
    local v = b and tonumber(b[key])
    if v == nil then return default end
    return v
  end
  return {
    eggLevel = num("eggLevel", Breeding.EGG_LEVEL),
    hatchHappiness = num("hatchHappiness", Breeding.HATCH_HAPPINESS),
    minStepsToEgg = num("minStepsToEgg", Breeding.MIN_STEPS_TO_EGG),
    withdrawFee = num("withdrawFee", Breeding.WITHDRAW_FEE),
    withdrawFeePerLevel = num("withdrawFeePerLevel",
      Breeding.WITHDRAW_FEE_PER_LEVEL),
  }
end
...
function Breeding.makeEgg(data, mon1, mon2, opts)
  ...
  local knobs = Breeding.knobs(data)
  local level = knobs.eggLevel
  ...
  local egg = Mon.new(data, species, level, {
    ...
    happiness = knobs.hatchHappiness,
  })
...
function Breeding.retrievePrice(grown, data)
  local knobs = Breeding.knobs(data)
  return knobs.withdrawFeePerLevel * math.max(0, grown or 0)
    + knobs.withdrawFee
end
...
function Breeding.canWithdraw(data, save, which)
  ...
  local price = Breeding.retrievePrice(grown, data)
...
function Breeding.initBreeding(data, save, opts)
  ...
  dc.compatible = true
  local steps
  local minSteps = Breeding.knobs(data).minStepsToEgg
  repeat steps = randomByte(opts.rng) until steps >= minSteps
  dc.stepsToEgg = steps
...
function Breeding.hatch(data, save, index, nickname)
  ...
  local knobs = Breeding.knobs(data)
  local hatchLevel = egg.level or knobs.eggLevel
  local hatched = Mon.new(data, egg.species, hatchLevel, {
    dvs = egg.dvs,
    moves = egg.moves,
    nickname = nickname,
    happiness = knobs.hatchHappiness,
  ...
  hatched.caughtLevel = hatchLevel
  party[index] = hatched
