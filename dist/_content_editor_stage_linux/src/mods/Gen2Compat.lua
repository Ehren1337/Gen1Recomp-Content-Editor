...
local function buildFieldDefaults()
  ...
  function FieldDefaults.fieldValue(data, key, ...)
    ...
    if key == "playerSprites" then
      ...
      local function idFor(slot)
        local state = ({
          walk = FieldMoves.PLAYER_NORMAL,
          bike = FieldMoves.PLAYER_BIKE,
          surf = FieldMoves.PLAYER_SURF,
          surfPikachu = FieldMoves.PLAYER_SURF_PIKA,
        })[slot]
        if state then return FieldMoves.spriteId(data, state) end
        return nil
      end
      if first == nil then
        return {
          ...
          bike = idFor("bike"),
          surf = idFor("surf"),
          surfPikachu = idFor("surfPikachu"),
        }
      end
      local id = idFor(first)
...
COVERAGE["src.world.FieldDefaults"] = {
  ...
  notes = {
    ...
    fieldValue = "serves Gold playerSprites via FieldMoves.spriteId "
      .. "(walk/bike/surf/surfPikachu, honoring data.gen2PlayerSprites)",
