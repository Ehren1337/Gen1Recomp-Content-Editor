...
-- Mount an absolute external directory onto the physfs read path.
-- append (default true): existing source wins name clashes; false prepends
-- so the mounted tree wins.  Used by the content editor to Link a Gen1Recomp
-- install's data/generated + assets/generated without copying the cache.
function CacheFs.mountExternal(dir, append)
  if not dir or dir == "" then return false end
  return mountReadable(dir, append)
end

function CacheFs.unmountExternal(dir)
  if not dir or dir == "" then return false end
...
function CacheFs.unmountVersion(version)
  ...
  if love.filesystem.unmount then
    ...
    -- mountGeneratedTrees overlays these onto un-prefixed paths; leave them
    -- up and the next version's require("data.generated.*") can still hit
    -- the previous game's files.
